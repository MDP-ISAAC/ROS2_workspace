...
class JointStateMonitor(Node):
    ...
    def send_gripper_command(self, command):
        """그리퍼 Open/Close 명령 발행 (0=open, 1=close)"""
        msg = Int32()
        msg.data = command
        self.gripper_cmd_pub.publish(msg)
        cmd_str = "OPEN" if command == 0 else "CLOSE"
        self.get_logger().info(f'Gripper command sent: {cmd_str} ({command})')
    ...
    def send_cartesian_command(self, x, y, z, roll, pitch, yaw, duration=0.5):
        ...
        # 상세 디버그 로그
        self.get_logger().info(f'━━━━━━━━━━ IK Request ━━━━━━━━━━')
        self.get_logger().info(f'Target Position: xyz=({x:.3f}, {y:.3f}, {z:.3f})')
        self.get_logger().info(f'Target Orientation: rpy=({math.degrees(roll):.1f}°, {math.degrees(pitch):.1f}°, {math.degrees(yaw):.1f}°)')
        self.get_logger().info(f'Target Quaternion: (x={qx:.4f}, y={qy:.4f}, z={qz:.4f}, w={qw:.4f})')
        self.get_logger().info(f'Current Joints: {[f"{math.degrees(j):.1f}°" for j in self.current_joints]}')
...
class MainWindow(QMainWindow):
    ...
    def init_ui(self):
        ...
        # 실시간 제어 컨텐츠 (접히는 부분)
        self.realtime_content = QWidget()
        self.realtime_content.setVisible(False)  # 초기에는 숨김
        realtime_content_layout = QVBoxLayout(self.realtime_content)
        
        # === Gripper Control (공통 영역 - 맨 위) ===
        gripper_control_group = QGroupBox('🤏 Gripper Control')
        gripper_control_layout = QVBoxLayout()
        
        # Gripper 슬라이더 (Joint Angle)
        gripper_slider_layout = QHBoxLayout()
        gripper_slider_layout.addWidget(QLabel('Gripper Angle:'))
        ...
        gripper_slider_layout.addWidget(self.gripper_slider)
        
        self.gripper_slider_label = QLabel('0.0°')
        self.gripper_slider_label.setFixedWidth(50)
        gripper_slider_layout.addWidget(self.gripper_slider_label)
        
        gripper_send_angle_btn = QPushButton('Send Gripper Angle')
        gripper_send_angle_btn.clicked.connect(self.send_realtime_gripper)
        gripper_slider_layout.addWidget(gripper_send_angle_btn)
        gripper_control_layout.addLayout(gripper_slider_layout)
        
        # Open/Close 버튼
        gripper_cmd_layout = QHBoxLayout()
        self.gripper_open_btn = QPushButton('Open Gripper')
        self.gripper_open_btn.clicked.connect(lambda: self.send_gripper_cmd(0))
        self.gripper_open_btn.setFixedHeight(30)
        gripper_cmd_layout.addWidget(self.gripper_open_btn)
        
        self.gripper_close_btn = QPushButton('Close Gripper')
        self.gripper_close_btn.clicked.connect(lambda: self.send_gripper_cmd(1))
        self.gripper_close_btn.setFixedHeight(30)
        gripper_cmd_layout.addWidget(self.gripper_close_btn)
        gripper_control_layout.addLayout(gripper_cmd_layout)
        
        gripper_control_group.setLayout(gripper_control_layout)
        self.gripper_control_group = gripper_control_group
        realtime_content_layout.addWidget(gripper_control_group)
        
        # 탭 위젯 생성 (Joint / Cartesian)
        ...
        realtime_content_layout.addWidget(self.control_tabs)
        
        # realtime_content를 realtime_layout에 추가
        realtime_layout.addWidget(self.realtime_content)
        ...
        capture_btn = QPushButton('Capture P')
        capture_btn.clicked.connect(self.capture_current_pose)
        capture_btn.setMaximumWidth(110)
        action_layout.addWidget(capture_btn)
        
        add_joint_btn = QPushButton('Joint P')
        add_joint_btn.clicked.connect(self.add_joint_pose)
        add_joint_btn.setMaximumWidth(90)
        action_layout.addWidget(add_joint_btn)
        
        add_cartesian_btn = QPushButton('Cartesian P')
        add_cartesian_btn.clicked.connect(lambda: self.add_action_to_sequence('CartesianPose'))
        add_cartesian_btn.setMaximumWidth(120)
        action_layout.addWidget(add_cartesian_btn)

        add_gripper_btn = QPushButton('Gripper')
        add_gripper_btn.clicked.connect(self.add_gripper_action)
        add_gripper_btn.setMaximumWidth(95)
        action_layout.addWidget(add_gripper_btn)
        
        add_gripper_open_btn = QPushButton('G Open')
        add_gripper_open_btn.clicked.connect(lambda: self.add_action_to_sequence('GripperOpen'))
        add_gripper_open_btn.setMaximumWidth(85)
        action_layout.addWidget(add_gripper_open_btn)
        
        add_gripper_close_btn = QPushButton('G Close')
        add_gripper_close_btn.clicked.connect(lambda: self.add_action_to_sequence('GripperClose'))
        add_gripper_close_btn.setMaximumWidth(85)
        action_layout.addWidget(add_gripper_close_btn)
        
        
        add_sleep_btn = QPushButton('Sleep')
        add_sleep_btn.clicked.connect(lambda: self.add_action_to_sequence('Sleep'))
        add_sleep_btn.setMaximumWidth(80)
        action_layout.addWidget(add_sleep_btn)
        ...
        play_selected_btn = QPushButton('▶️ Play')
        play_selected_btn.clicked.connect(self.play_selected_block)
        edit_layout.addWidget(play_selected_btn)
        ...
        # Splitter에 추가 (Sequence 왼쪽, Blocks 오른쪽)
    ...
    def toggle_realtime_mode(self, state):
        """실시간 제어 모드 토글 - 패널 표시/숨김"""
        self.realtime_mode = (state == Qt.Checked)
        
        # 실시간 제어 컨텐츠 표시/숨김
        self.realtime_content.setVisible(self.realtime_mode)
    ...
    def send_realtime_gripper(self):
        ...
        self.status_label.setText(f'Sent gripper angle: {gripper_deg:.1f}° ({gripper_rad:.4f} rad)')
    
    def send_gripper_cmd(self, command):
        """그리퍼 Open/Close 명령 전송 (0=open, 1=close)"""
        if not self.realtime_mode:
            return
        
        self.ros_thread.node.send_gripper_command(command)
        cmd_str = "OPEN" if command == 0 else "CLOSE"
        self.status_label.setText(f'🤏 Sent gripper command: {cmd_str}')
    ...
    def add_joint_pose(self):
        ...
        if dialog.exec_():
            ...
            joint_data = {'name': name, 'joints': joints, 'duration': duration}
            self.sequence.append(('joint', joint_data))
    ...
    def add_gripper_action(self):
        ...
        if dialog.exec_():
            ...
            gripper_data = {'name': name, 'gripper': gripper_rad, 'duration': duration}
            self.sequence.append(('gripper', gripper_data))
            self.update_sequence_list()
    ...
    def edit_sequence_item(self):
        ...
        if item_type == 'joint':
        ...
        elif item_type == 'CartesianPose':
            # Cartesian Pose 편집 다이얼로그
            dialog = QDialog(self)
            dialog.setWindowTitle('Edit Cartesian Pose')
            layout = QVBoxLayout()
            ...
            name_layout.addWidget(QLabel('Name:'))
            name_edit = QLineEdit()
            name_edit.setText(data.get('name', 'Cartesian'))
            ...
            x_spin.setValue(data['x'])
            x_spin.setSuffix(' m')
            pos_grid.addWidget(QLabel('X:'), 0, 0)
            pos_grid.addWidget(x_spin, 0, 1)
            
            y_spin = QDoubleSpinBox()
            y_spin.setRange(-1.0, 1.0)
            y_spin.setSingleStep(0.01)
            y_spin.setDecimals(3)
            y_spin.setValue(data['y'])
            y_spin.setSuffix(' m')
            pos_grid.addWidget(QLabel('Y:'), 0, 2)
            pos_grid.addWidget(y_spin, 0, 3)
            
            z_spin = QDoubleSpinBox()
            z_spin.setRange(0.0, 1.0)
            z_spin.setSingleStep(0.01)
            z_spin.setDecimals(3)
            z_spin.setValue(data['z'])
            z_spin.setSuffix(' m')
            pos_grid.addWidget(QLabel('Z:'), 0, 4)
            pos_grid.addWidget(z_spin, 0, 5)
            ...
            roll_spin.setValue(data['roll'])
            roll_spin.setSuffix(' °')
            ori_grid.addWidget(QLabel('Roll:'), 0, 0)
            ori_grid.addWidget(roll_spin, 0, 1)
            
            pitch_spin = QDoubleSpinBox()
            pitch_spin.setRange(-180.0, 180.0)
            pitch_spin.setSingleStep(5.0)
            pitch_spin.setDecimals(1)
            pitch_spin.setValue(data['pitch'])
            pitch_spin.setSuffix(' °')
            ori_grid.addWidget(QLabel('Pitch:'), 0, 2)
            ori_grid.addWidget(pitch_spin, 0, 3)
            
            yaw_spin = QDoubleSpinBox()
            yaw_spin.setRange(-180.0, 180.0)
            yaw_spin.setSingleStep(5.0)
            yaw_spin.setDecimals(1)
            yaw_spin.setValue(data['yaw'])
            yaw_spin.setSuffix(' °')
            ori_grid.addWidget(QLabel('Yaw:'), 0, 4)
            ori_grid.addWidget(yaw_spin, 0, 5)
            layout.addLayout(ori_grid)
            ...
            if dialog.exec_() == QDialog.Accepted:
                ...
                cartesian_data = {
                    ...
                    'y': y_spin.value(),
                    'z': z_spin.value(),
                    'roll': roll_spin.value(),
                    'pitch': pitch_spin.value(),
                    'yaw': yaw_spin.value(),
                    'duration': duration_spin.value()
                }
                self.sequence[current_row] = ('CartesianPose', cartesian_data)
                self.update_sequence_list()
                self.status_label.setText(f'Edited: {name}')
            ...
            duration, ok = QInputDialog.getDouble(
                self, 'Edit Sleep Duration', 
                'Enter sleep time (seconds):', 
                data['duration'], 0.1, 10.0, 1
            )
            if ok:
                self.sequence[current_row] = ('Sleep', {'duration': duration})
                self.update_sequence_list()
                self.status_label.setText(f'Edited Sleep: {duration}s')
        
        elif item_type in ['GripperOpen', 'GripperClose']:
            # GripperOpen/Close는 편집할 내용이 없음
            QMessageBox.information(self, 'Info', f'{item_type} has no editable properties.')
            return
    ...
    def update_sequence_list(self):
        ...
        for i, (item_type, data) in enumerate(self.sequence):
            ...
            elif item_type == 'GripperOpen':
                text = "🟢 [GRIPPER OPEN]"
            elif item_type == 'GripperClose':
                text = "🔴 [GRIPPER CLOSE]"
            elif item_type == 'CartesianPose':
                name = data.get('name', 'Cartesian')
                text = f"🗺️ [CARTESIAN] {name} ({data['duration']:.1f}s): X={data['x']:.3f}, Y={data['y']:.3f}, Z={data['z']:.3f}"
            elif item_type == 'Sleep':
                text = f"⏱️ [SLEEP] {data['duration']:.1f}s"
    ...
    def _play_sequence_thread(self):
        ...
        for i, (item_type, data) in enumerate(self.sequence):
            ...
            if item_type == 'joint':
                ...
                # Gripper Open 명령
                self.ros_thread.node.send_gripper_command(0)
                self.status_label.setText(f"Playing [{i+1}/{total_items}]: Gripper Open")
                time.sleep(0.5)  # 0.5초 대기
            
            elif item_type == 'GripperClose':
                # Gripper Close 명령
                self.ros_thread.node.send_gripper_command(1)
                self.status_label.setText(f"Playing [{i+1}/{total_items}]: Gripper Close")
                time.sleep(0.5)  # 0.5초 대기
            
            elif item_type == 'CartesianPose':
                # Cartesian Pose로 IK 계산 후 전송
                self.status_label.setText(f"Playing [{i+1}/{total_items}]: Computing IK...")
                
                # IK 계산
                target_pose = Pose()
                target_pose.position = Point(x=data['x'], y=data['y'], z=data['z'])
                ...
                request.robot_state.joint_state.name = ['joint_1', 'joint_2', 'joint_3', 'joint_4', 'joint_5', 'joint_6']
                request.robot_state.joint_state.position = [0.0] * 6
                request.pose_stamped.header.frame_id = 'base_link'
                request.pose_stamped.pose = target_pose
                request.timeout.sec = 5
                
                ik_request_msg = GetPositionIK.Request()
                ik_request_msg.ik_request = request
                
                future = self.ros_thread.node.ik_client.call_async(ik_request_msg)
                rclpy.spin_until_future_complete(self.ros_thread.node, future, timeout_sec=5.0)
                
                if future.result() and future.result().error_code.val == 1:
                    joint_solution = list(future.result().solution.joint_state.position)
                    self.ros_thread.node.send_trajectory([joint_solution], [data['duration']])
                    self.status_label.setText(f"Playing [{i+1}/{total_items}]: Cartesian move")
                    time.sleep(data['duration'])
                else:
                    self.status_label.setText(f"❌ IK failed for Cartesian pose [{i+1}]")
                    time.sleep(1.0)
            
            elif item_type == 'Sleep':
                # Sleep
                self.status_label.setText(f"Playing [{i+1}/{total_items}]: Sleep {data['duration']:.1f}s")
                time.sleep(data['duration'])
    ...
    def play_selected_block(self):
        """선택한 블록만 재생"""
        current_row = self.sequence_list.currentRow()
        
        if current_row < 0:
            QMessageBox.warning(self, 'Warning', 'No block selected!')
            return
        ...
        if self.is_playing:
            QMessageBox.warning(self, 'Warning', 'Already playing!')
            return
        ...
        self.play_thread = threading.Thread(
            target=self._play_selected_block_thread, 
            args=(current_row,),
            daemon=True
        )
        self.play_thread.start()
    
    def _play_selected_block_thread(self, block_index):
        """선택한 블록 재생 스레드"""
        if block_index >= len(self.sequence):
            self.status_label.setText('❌ Invalid block index')
            self.is_playing = False
            self.play_btn.setEnabled(True)
            self.stop_btn.setEnabled(False)
            return
        
        item_type, data = self.sequence[block_index]
        
        if item_type == 'joint':
            # Joint 포즈 전송
            self.ros_thread.node.send_trajectory([data['joints']], [data['duration']])
            self.status_label.setText(f"▶️ Playing: {data['name']}")
            time.sleep(data['duration'])
            ...
            # Gripper Open 명령
            self.ros_thread.node.send_gripper_command(0)
            self.status_label.setText(f"▶️ Playing: Gripper Open")
            time.sleep(0.5)
        
        elif item_type == 'GripperClose':
            # Gripper Close 명령
            self.ros_thread.node.send_gripper_command(1)
            self.status_label.setText(f"▶️ Playing: Gripper Close")
            time.sleep(0.5)
        
        elif item_type == 'CartesianPose':
            # Cartesian Pose로 IK 계산 후 전송
            self.status_label.setText(f"▶️ Playing: Computing IK...")
            ...
            if future.result() and future.result().error_code.val == 1:
                joint_solution = list(future.result().solution.joint_state.position)
                self.ros_thread.node.send_trajectory([joint_solution], [data['duration']])
                self.status_label.setText(f"▶️ Playing: Cartesian move")
                time.sleep(data['duration'])
            else:
                self.status_label.setText(f"❌ IK failed for Cartesian pose")
                time.sleep(1.0)
        
        elif item_type == 'Sleep':
            # Sleep
            self.status_label.setText(f"▶️ Playing: Sleep {data['duration']:.1f}s")
            time.sleep(data['duration'])
        
        self.is_playing = False
        self.play_btn.setEnabled(True)
        self.stop_btn.setEnabled(False)
        self.status_label.setText('✅ Selected block completed')
    ...
    def save_selected_as_block(self):
        ...
        # 데이터에서 이름 가져오기 (없으면 기본 이름)
        if 'name' in data:
            name = data['name']
        else:
            # GripperOpen, GripperClose, Sleep 등
            name = item_type
        ...
        filename = f'{prefix} {clean_name}.json'
        ...
        try:
            ...
            self.status_label.setText(f'Saved block: {prefix} {name}')
        except Exception as e:
    ...
    def add_action_to_sequence(self, action_type):
        """Action 블록을 시퀀스에 추가"""
        if action_type == 'GripperOpen':
            self.sequence.append(('GripperOpen', {}))
            self.update_sequence_list()
            self.status_label.setText('Added GripperOpen to sequence')
        
        elif action_type == 'GripperClose':
            self.sequence.append(('GripperClose', {}))
            self.update_sequence_list()
            self.status_label.setText('Added GripperClose to sequence')
        
        elif action_type == 'CartesianPose':
            # Cartesian Pose 다이얼로그
            dialog = QDialog(self)
            dialog.setWindowTitle('Add Cartesian Pose')
            layout = QVBoxLayout()
            ...
            name_layout.addWidget(QLabel('Name:'))
            name_edit = QLineEdit()
            name_edit.setText(f'Cartesian {len(self.sequence)+1}')
            name_layout.addWidget(name_edit)
            layout.addLayout(name_layout)
            ...
            x_spin.setSingleStep(0.01)
            x_spin.setDecimals(3)
            x_spin.setValue(self.x_spin.value())
            x_spin.setSuffix(' m')
            pos_grid.addWidget(QLabel('X:'), 0, 0)
            pos_grid.addWidget(x_spin, 0, 1)
            ...
            y_spin.setSingleStep(0.01)
            y_spin.setDecimals(3)
            y_spin.setValue(self.y_spin.value())
            y_spin.setSuffix(' m')
            pos_grid.addWidget(QLabel('Y:'), 0, 2)
            pos_grid.addWidget(y_spin, 0, 3)
            ...
            z_spin.setSingleStep(0.01)
            z_spin.setDecimals(3)
            z_spin.setValue(self.z_spin.value())
            z_spin.setSuffix(' m')
            pos_grid.addWidget(QLabel('Z:'), 0, 4)
            pos_grid.addWidget(z_spin, 0, 5)
            layout.addLayout(pos_grid)
            ...
            roll_spin.setSingleStep(5.0)
            roll_spin.setDecimals(1)
            roll_spin.setValue(self.roll_spin.value())
            roll_spin.setSuffix(' °')
            ori_grid.addWidget(QLabel('Roll:'), 0, 0)
            ori_grid.addWidget(roll_spin, 0, 1)
            ...
            pitch_spin.setSingleStep(5.0)
            pitch_spin.setDecimals(1)
            pitch_spin.setValue(self.pitch_spin.value())
            pitch_spin.setSuffix(' °')
            ori_grid.addWidget(QLabel('Pitch:'), 0, 2)
            ori_grid.addWidget(pitch_spin, 0, 3)
            ...
            yaw_spin.setSingleStep(5.0)
            yaw_spin.setDecimals(1)
            yaw_spin.setValue(self.yaw_spin.value())
            yaw_spin.setSuffix(' °')
            ori_grid.addWidget(QLabel('Yaw:'), 0, 4)
            ori_grid.addWidget(yaw_spin, 0, 5)
            layout.addLayout(ori_grid)
            ...
            if dialog.exec_() == QDialog.Accepted:
                name = name_edit.text().strip()
                if not name:
                    name = f'Cartesian {len(self.sequence)+1}'
                ...
                self.sequence.append(('CartesianPose', cartesian_data))
                self.update_sequence_list()
                self.status_label.setText(f'Added CartesianPose: {name}')
            ...
            if ok:
                self.sequence.append(('Sleep', {'duration': duration}))
                self.update_sequence_list()
                self.status_label.setText(f'Added Sleep ({duration}s) to sequence')
    ...
    def update_blocks_list(self):
        ...
        for filename, (item_type, data) in sorted(self.single_poses.items()):
            # 파일명에서 .json 제거하고 언더스코어를 공백으로 (파일명에 이미 [JOINT] 등의 접두사 포함)
